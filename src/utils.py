...
def edge_match(edge_sco, rand_num):
    top = 0
    bot = edge_sco.size(0)-1
    half = int((bot+top)/2)
    while top != bot:
        if rand_num < edge_sco[half]:
            bot = half
            half = int((bot + top) / 2)
        else:
            top = half+1
...
def non_uni_sample(row, col, n_v, n_t):
    deg = torch.zeros(torch.max(col)+1)
    for i in range(row.size(0)):
        deg[row[i]] = deg[row[i]] + 1
        deg[col[i]] = deg[col[i]] + 1
    edge_mask = torch.ones(row.size(0), dtype=torch.bool)
    edge_wei = torch.zeros(row.size(0), dtype=torch.float)
    for i in range(row.size(0)):
        edge_wei[i] = 1 / (deg[row[i]]*deg[col[i]])
    wei_sum = torch.sum(edge_wei)
    edge_sco = torch.zeros(row.size(0), dtype=torch.float)
    edge_sco[0] = edge_wei[0] / wei_sum
    for i in range(row.size(0)-1):
        edge_sco[i+1] = edge_sco[i] + edge_wei[i+1]/wei_sum
    for num_test in range(n_t):
        select = 0
        while select == 0:
            rand_num = torch.rand(1)
            edge_index = edge_match(edge_sco, rand_num)
            if edge_mask[edge_index] == True:
                select = 1
                edge_mask[edge_index] = False
    row_rest = row[edge_mask]
    col_rest = col[edge_mask]
    perm = torch.randperm(row_rest.size(0))
    row_rest, col_rest = row_rest[perm], col_rest[perm]
    r_v, c_v = row_rest[:n_v], col_rest[:n_v]
    r_t, c_t = row_rest[n_v:], col_rest[n_v:]
    return torch.stack([r_t, c_t], dim=0), torch.stack([r_v, c_v], dim=0), torch.stack([row[torch.logical_not(edge_mask)], col[torch.logical_not(edge_mask)]], dim=0)

def split_edges_gae(data,args):
    ...
    if args.non_uni:
        data.train_pos, data.val_pos, data.test_pos = non_uni_sample(row, col, n_v, n_t)
    else:
    ...
    neg_adj_mask[row, col] = 0
    neg_row, neg_col = neg_adj_mask.nonzero(as_tuple=False).t()
    perm = torch.randperm(neg_row.size(0))[:n_v]
    data.val_neg = torch.stack([neg_row[perm], neg_col[perm]], dim=0)

    adj = torch.zeros(data.num_nodes, data.num_nodes, dtype=torch.uint8)
    adj[row, col] = 1
    adj[col, row] = 1
    n_train = data.train_pos.size(1)

    adj2 = torch.mm(adj, adj)

    tneg_row, tneg_col = neg_adj_mask.nonzero(as_tuple=False).t()
    perm = torch.randperm(tneg_row.size(0))[:n_train]

    data.train_neg = torch.stack([tneg_row[perm], tneg_col[perm]], dim=0)

    return data, adj2, tneg_row, tneg_col

def set_init_attribute_representation_gae(data,args):
    ...
    neg_edge_index = data.train_neg
    #generate the initial node representation using unsupervised models
    data_observed.x = x
    feature_results = None

    return data_observed, feature_results, edge_index_observed, x, val_and_test,  neg_edge_index

def prepare_data_gae(args):
    ...
    if args.data_name in ('cora', 'citeseer', 'pubmed'):
        ...
        data, adj2, tneg_row, tneg_col = split_edges_gae(data, args)
        ...
        if args.use_splitted == True:  # use splitted train/val/test
            ...
            data, adj2, tneg_row, tneg_col = split_edges_gae(data, args)
    set_random_seed(args.seed)
    data_observed, feature_results, edge_index_observed, x, val_and_test, neg_edge_index = set_init_attribute_representation_gae(
        data, args)

    return data.train_pos, edge_index_observed, x, val_and_test, neg_edge_index, adj2, tneg_row, tneg_col
...
def plus_edge_wp(data_observed, label, p_edge, args):
...
def minus_edge_wp(data_observed, label, p_edge, args):
...
def set_init_attribute_representation_wp(data,args, node_deg, neg_row, neg_col):
    #Construct data_observed and compute its node attributes & representation
    edge_index = torch.cat((data.train_pos,data.train_pos[[1,0],:]),dim=1)
    if args.observe_val_and_injection == False:
        data_observed = Data(edge_index=edge_index)
...
def prepare_datawp(args):
    ...
    set_random_seed(args.seed)
    data_observed, feature_results = set_init_attribute_representation_wp(data, args, node_deg, neg_row, neg_col)
    ...
    for i in range(data.train_pos.size(1)):
        train_graphs.append(minus_edge_wp(data_observed, 1, data.train_pos[:, i], args))

    for i in range(data.train_neg.size(1)):
        train_graphs.append(plus_edge_wp(data_observed, 0, data.train_neg[:, i], args))

    for i in range(data.test_pos.size(1)):
        test_graphs.append(plus_edge_wp(data_observed, 1, data.test_pos[:, i], args))

    for i in range(data.test_neg.size(1)):
        test_graphs.append(plus_edge_wp(data_observed, 0, data.test_neg[:, i], args))
    if args.observe_val_and_injection == False:
        for i in range(data.val_pos.size(1)):
            val_graphs.append(plus_edge_wp(data_observed, 1, data.val_pos[:, i], args))

        for i in range(data.val_neg.size(1)):
            val_graphs.append(plus_edge_wp(data_observed, 0, data.val_neg[:, i], args))
    else:
        for i in range(data.val_pos.size(1)):
            val_graphs.append(minus_edge_wp(data_observed, 1, data.val_pos[:, i], args))
