...
class LinkPred(MessagePassing):
    def __init__(self, in_channels: int, hidden_channels: int, heads: int = 1,\
                 walk_len: int = 6, z_max: int =100, MSE: bool=True):
        super(LinkPred, self).__init__()

        self.conv1 = GCNConv(in_channels, hidden_channels)
        self.conv2 = GCNConv(hidden_channels, hidden_channels)

        #self.wp = WalkPooling(in_channels + hidden_channels*2, hidden_channels, heads, walk_len)
    ...
    def forward(self, x, edge_index, edge_mask, batch, z = None, gpu_num = '0'):
...
class WalkPooling(MessagePassing):
    ...
    def attention_mlp(self, x, edge_index):
        ...
        #key = self.lin_key1(x).reshape(-1, self.hidden_channels)
...
class MLP(torch.nn.Module):
    ...
    def __init__(self,input_size,MSE=True):
        ...
        #self.linear1 = torch.nn.Linear(input_size, input_size * 10)
        #self.linear5 = torch.nn.Linear(input_size * 10, 1)
        self.act= nn.ReLU()
        self.MSE=MSE
    def forward(self, x):
        out= self.nn(x)
